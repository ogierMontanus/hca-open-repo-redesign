#!/usr/bin/env python3
"""
apply_embedded_name_splits.py
---------------------------------
Applies the SAFE rows from suggest_embedded_name_splits.py: 09_description
is trimmed back to its "head" (this row's own remaining description,
before the first embedded fused name), and each tail fragment becomes a
new standardpost.

Run from the repo root, AFTER suggest_embedded_name_splits.py:
  python scripts/segmentation/archive/apply_embedded_name_splits.py
"""
import csv
import os
import re
...
REVIEW_TSV = os.path.join(ROOT, "data", "review", "personregister_xi_embedded_name_review.tsv")
...
def main():
    ...
    for row in rows:
        ...
        tails = [f.strip() for f in spec["tail_fragments"].split(" || ") if f.strip()]
        matches = [m.strip() for m in spec["xlsx_matches"].split(" || ") if m.strip()]

        row = dict(row)
        row["09_description"] = spec["head"]
        out.append(row)

        for frag, xl_title in zip(tails, matches):
    ...
    print(f"applied {len(safe)} SAFE embedded-name splits -> {n_new} new entries")
